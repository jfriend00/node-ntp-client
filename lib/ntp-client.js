...
(function (exports) {
    ...
    exports.getNetworkTime = function (server, port, callback) {
        ...
        // Utility function to close the socket and ignore errors upon closing
        // dgram may throw if client.close() is called, but the original
        // socket creation did not work correctly (e.g. DNS down, bad host name, etc...)
        // These errors are already reported via callbacks, we don't want an 
        // uncaught exception here
        function close() {
            try {
                client.close();
            } catch(e) {
                // client.close() may throw, ignore that
            }
        }
